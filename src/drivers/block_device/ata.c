...
static struct ata_disk ata_disk_primary_master = { .io_base = 0x1F0,
						   .control_base = 0x3F6,
						   .irq_num = 14 + 0x20,
						   .is_master = true,
						   .name = "hd0:0" };
static struct ata_disk ata_disk_primary_slave = { .io_base = 0x1F0,
						  .control_base = 0x3F6,
						  .irq_num = 14 + 0x20,
						  .is_master = false,
						  .name = "hd0:1" };
static struct ata_disk ata_disk_secondary_master = { .io_base = 0x170,
						     .control_base = 0x376,
						     .irq_num = 15 + 0x20,
						     .is_master = true,
						     .name = "hd1:0" };
static struct ata_disk ata_disk_secondary_slave = { .io_base = 0x170,
						    .control_base = 0x376,
						    .irq_num = 15 + 0x20,
						    .is_master = false,
						    .name = "hd1:1" };
...
static LIST_HEAD(active_ata_devices);
...
static size_t read_one_sector_28(struct ata_disk *disk, uint32_t lba28, void *buffer)
{
	...
	outb(disk->io_base + ATA_REG_HDDEVSEL,
	     (disk->is_master == true ? 0xe0 : 0xf0) | (uint8_t)((lba28 & 0x0f000000) >> 24));
...
static void disk_init(struct ata_disk *disk)
{
	...
	if (cl == 0xFF && ch == 0xFF) {
		...
		identify_disk(disk);
		list_add_tail(&disk->list, &active_ata_devices);
...
static void debug_device()
{
	...
	list_for_each_entry (cur, &active_ata_devices, list) {
		printf("%s (%s)\n", cur->info.model, cur->type == (PATA) ? "PATA" : "PATAPI");
	}
}
