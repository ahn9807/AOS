...
static int ext2_mount(device_t *device, struct inode *super_node, void *aux)
{
	...
	// Read group descs
	dev_read(device, fs->block_size == 1024 ? 2048 : fs->block_size, fs->desc_len * sizeof(ext2_group_desc_t),
		 fs->group_descs);
...
static int sync_gdesc(ext2_fs_t *ext2)
{
	dev_write(ext2->mountpoint->device, ext2->block_size == 1024 ? 2048 : ext2->block_size,
		  ext2->desc_len * sizeof(ext2_group_desc_t), ext2->group_descs);
	return 0;
}
...
static uint32_t block_offset(ext2_fs_t *ext2, ext2_inode_t *inode, uint32_t block_idx)
{
	...
	b = a - p;
	if (b < 0) {
		dev_read(ext2->mountpoint->device, SINGLE_INDIRECT_POINTER(inode) * ext2->block_size, ext2->block_size,
			 tmp);
		ret = tmp[a];
		goto done;
	}
	...
	if (c < 0) {
		c = b / p;
		d = b - c * p;
		dev_read(ext2->mountpoint->device, DOUBLE_INDIRECT_POINTER(inode) * ext2->block_size, ext2->block_size,
			 tmp);
	...
	if (d < 0) {
		...
		f = (c - e * p * p) / p;
		g = (c - e * p * p - f * p);
		dev_read(ext2->mountpoint->device, TRIPLE_INDIRECT_POINTER(inode) * ext2->block_size, ext2->block_size,
			 tmp);
...
static int alloc_inode(ext2_fs_t *ext2)
{
	...
	for (int i = 0; i < ext2->desc_len; i++) {
		if (ext2->group_descs[i].free_inodes_count > 0) {
			dev_read(ext2->mountpoint->device, ext2->group_descs[i].inode_bitmap, ext2->block_size,
				 bitmap_get_raw(imap));
	...
	dev_write(ext2->mountpoint->device, ext2->group_descs[group_idx].inode_bitmap, ext2->block_size,
		  bitmap_get_raw(imap));
...
static int free_inode(ext2_fs_t *ext2, uint32_t inode_ptr)
{
	...
	dev_read(ext2->mountpoint->device, ext2->group_descs[group_idx].inode_bitmap, ext2->block_size,
		 bitmap_get_raw(imap));
...
static int alloc_block(ext2_fs_t *ext2)
{
	...
	for (int i = 0; i < ext2->desc_len; i++) {
		if (ext2->group_descs[i].free_blocks_count > 0) {
			dev_read(ext2->mountpoint->device, ext2->group_descs[i].block_bitmap, ext2->block_size,
				 bitmap_get_raw(bmap));
	...
	dev_write(ext2->mountpoint->device, ext2->group_descs[group_idx].block_bitmap, ext2->block_size,
		  bitmap_get_raw(bmap));
...
static int free_block(ext2_fs_t *ext2, uint32_t block_ptr)
{
	...
	dev_read(ext2->mountpoint->device, ext2->group_descs[group_idx].block_bitmap, ext2->block_size,
		 bitmap_get_raw(bmap));
