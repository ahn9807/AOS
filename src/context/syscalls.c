...
void syscall_init()
{
	...
	for (syscall_info_t **cursor = (syscall_info_t **)&_start_syscall_info_table;
	     (uintptr_t)cursor < (uintptr_t)&_end_syscall_info_table;
	     cursor = (syscall_info_t **)((uintptr_t)cursor + (uintptr_t)sizeof(syscall_info_t **))) {
		// sys_ni
...
void syscall_handler(struct intr_frame *if_)
{
	if (if_->reg.rax > MAX_SYSCALL_NR) {
		...
		case 3:
			...
			break;
		case 4:
			ret = syscall_info->syscall_p.syscall_arg4(if_->reg.rdi, if_->reg.rsi, if_->reg.rdx,
								   if_->reg.r10);
			break;
		case 5:
			ret = syscall_info->syscall_p.syscall_arg5(if_->reg.rdi, if_->reg.rsi, if_->reg.rdx,
								   if_->reg.r10, if_->reg.r8);
			break;
		case 6:
			ret = syscall_func_table[if_->reg.rax].syscall_arg6(if_->reg.rdi, if_->reg.rsi, if_->reg.rdx,
									    if_->reg.r10, if_->reg.r8, if_->reg.r9);
			break;
		default:
...
static void debug_syscall(struct intr_frame *if_)
{
	...
	printf("rax: %d rdi: 0x%x, rsi: 0x%x,\nrdx: 0x%x, r10: 0x%x, r8: 0x%x, r9: 0x%x\n", if_->reg.rax, if_->reg.rdi,
	       if_->reg.rsi, if_->reg.rdx, if_->reg.r10, if_->reg.r8, if_->reg.r9);
}